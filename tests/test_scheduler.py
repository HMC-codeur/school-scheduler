...
from backend.services.scoring import compute_schedule_signature
...
def test_fast_valid_attempt_is_complete_and_deterministic():
    ...
    first = SchedulerService.generate(classes, teachers, subjects, slots, generation_seed=7, quality_attempts=1)
    second = SchedulerService.generate(classes, teachers, subjects, slots, generation_seed=7, quality_attempts=1)

    assert first.success is True
    assert first.scheduled_sessions == first.required_sessions
    assert first.conflicts_count == 0
    assert compute_schedule_signature(first.schedule) == compute_schedule_signature(second.schedule)
...
def test_score_breakdown_aggregates_repeated_penalties():
    classes = [Class(id=1, name="A")]
    subjects = [Subject(name="Math", hours_per_week=5)]
    teachers = [Teacher(id=1, name="T1", subjects=["Math"])]
    slots = ["Mon-08:00", "Mon-09:00", "Mon-10:00", "Mon-11:00", "Mon-13:00"]

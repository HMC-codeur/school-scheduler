...
function bindForms() {
  ...
  ["condition-teacher-name", "condition-class-name", "condition-subject-name"].forEach((id) => document.getElementById(id).addEventListener("input", buildConditionText));
  document.getElementById("condition-slot").addEventListener("change", buildConditionText);
...
function updateProgressStatus({ classes, teachers, subjects, slots, conditions }) {
  const items = [
    ["progress-classes", "Classes", classes],
    ["progress-teachers", "Professeurs", teachers],
    ["progress-subjects", "Matières", subjects],
    ["progress-slots", "Créneaux", slots],
    ["progress-conditions", "Conditions", conditions],
  ];
  items.forEach(([id, label, count]) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.textContent = `${label}: ${count}`;
    el.classList.toggle("ready", Number(count) > 0);
  });
}
async function runAction(buttonId, path, loadingLabel) { const btn = document.getElementById(buttonId); setLoading(btn, true, loadingLabel); try { const res = await api(path, { method: "POST" }); notify(res.message || "Done", res.success === false ? "error" : "success"); await refresh(); if (path === "/schedule/clear") { document.getElementById("demo-summary").textContent = "Aucune démo volumineuse chargée."; lastQualityMetrics = null; document.getElementById("generation-status").textContent = "Aucune génération lancée pour le moment."; } } catch (error) { notify(error.message, "error"); } finally { setLoading(btn, false); } }
async function runGenerateSchedule() { const btn = document.getElementById("generate-btn"); setLoading(btn, true, "Generating..."); try { const res = await api("/schedule/generate", { method: "POST" }); if (res.success === false) throw new Error(res.message || "Failed to generate schedule"); lastQualityMetrics = res; renderQualityMetrics(lastQualityMetrics); await refreshScheduleTable(); notify(res.message || "Emploi du temps généré avec succès"); document.getElementById("generation-status").textContent = `Dernière génération : ${new Date().toLocaleString("fr-FR")}.`; } catch (error) { notify(`Échec de génération : ${error.message}`, "error"); } finally { setLoading(btn, false); } }
...
async function refresh() { const [classes, subjects, teachers, slots, schedule, conditions, timeSettings] = await Promise.all([api("/classes"), api("/subjects"), api("/teachers"), api("/slots"), api("/schedule"), api("/conditions"), api("/time-settings")]); document.getElementById("count-classes").textContent = classes.length; document.getElementById("count-subjects").textContent = subjects.length; document.getElementById("count-teachers").textContent = teachers.length; document.getElementById("count-slots").textContent = slots.length; updateProgressStatus({ classes: classes.length, teachers: teachers.length, subjects: subjects.length, slots: slots.length, conditions: conditions.length }); fillList("classes-list", classes.map((x) => `${x.name} (max/day: ${x.max_lessons_per_day})`)); fillList("subjects-list", subjects.map((x) => `${x.name} (${x.hours_per_week}h)`)); fillList("teachers-list", teachers.map((x) => `${x.name}: ${x.subjects.join(", ")} | max/day: ${x.max_lessons_per_day} | unavailable: ${x.unavailable_slots.join(", ") || "-"}`)); fillList("slots-list", slots); renderConditionsList(conditions); fillTimeSettingsForm(timeSettings); populateUnavailableSlots(slots); scheduleState.slots = slots; scheduleState.classes = classes.map((c) => c.name); scheduleState.teachers = teachers.map((t) => t.name); scheduleState.schedule = schedule || {}; populateScheduleFilters(); renderScheduleTableFromState(); renderQualityMetrics(lastQualityMetrics || {}); updateConditionFieldVisibility(); }
